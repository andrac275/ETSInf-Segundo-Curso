...
int spinlock = 0;   //al estar a 0 es que es FALSE i que la seccio critica NO esta ocupada.
...
void *inc (void *parameter) {
    ...
    while(test_and_set(&spinlock)){/*Bucle buit*/;} /*ProtocolEntrada*/
    
    for (cont = 0; cont < REPETITIONS; cont = cont + 1) {  
        V = V + 1;/*Zona critica*/          
    }
    
    spinlock = 0; /*ProtocolEixida*/
