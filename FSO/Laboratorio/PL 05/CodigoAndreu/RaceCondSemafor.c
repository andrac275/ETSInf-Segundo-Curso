...
#include <pthread.h>
#include <stdlib.h>
#include <semaphore.h>  //Llibreria Semafor

#define REPETITIONS 20000000   // CONSTANT

// GLOBAL SHARED VARIABLES 
long int V = 100;      // Initial value
sem_t semafor;  //Variable del semafor declarada, sense incialitzar.
...
void *inc (void *parameter) {
    ...
    for (cont = 0; cont < REPETITIONS; cont = cont + 1) {  
        sem_wait(&semafor); /*<- Seccio entrada*/
            V = V + 1; /*<- Seccio critica*/
        sem_post(&semafor); /*<- Seccio salida*/
