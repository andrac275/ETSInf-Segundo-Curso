    ...
    */
    ...
    private static <T extends Comparable<T>> T[] mergeSort2(T[] v,
        ...
        if(f - i == 1){//Talla 2
            ...
            if(v[i].compareTo(v[f]) < 0){// Dos elements ordenats
                T[] aux = (T[]) new Comparable[2];
                aux[0] = v[i]; 
                aux[1] = v[f];
                return aux;
            }else{//Intercanviar posicions
                T[] aux = (T[]) new Comparable[2];
                aux[0] = v[f]; 
                aux[1] = v[i];
                return aux;
    ...
    private static <T extends Comparable<T>> T[] merge2(T[] v1, T[] v2) {
        ...
        //Estos dos son per si quedara algun element per fer en algun dels dos arrays.
        while (i <= v1.length -1) { aux[k++] = v1[i++]; }
        while (j <= v2.length -1) { aux[k++] = v2[j++]; }
        
        //Retorna array ordenat.
        return aux;
    }
