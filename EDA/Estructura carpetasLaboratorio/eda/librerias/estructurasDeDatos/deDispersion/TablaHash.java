...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    protected final void rehashing() {
        //Calcular nova capacitat
        int novaCapacitat = siguientePrimo(elArray.length * 2);
        //Guardar elArray antic i les seues entrades
        ListaConPI<EntradaHash<C, V>>[] elArrayAntic = this.elArray;
        //Inicialitzar un nou elArray amb la nova capacitat
        this.elArray = new LEGListaConPI[novaCapacitat];
        for (int i = 0; i < elArray.length; i++) {
            elArray[i] = new LEGListaConPI<EntradaHash<C, V>>();
        }
        ...
        //Reiniciar la talla
        this.talla = 0;
        //Tornar a inserir tots els elements a la taula        
        for(int cub=0; cub < elArrayAntic.length; cub++){//Per cada cubeta a elArrayAntic
            ListaConPI<EntradaHash<C, V>> cubeta = elArrayAntic[cub];
            
            for(cubeta.inicio(); !cubeta.esFin(); cubeta.siguiente()){//Per cada EntradaHash a la cubeta
                //Inserir la clau i el valor a la nova taula
                
                //EntradaHash<C,V> aux =cubeta.recuperar(); insertar(aux.clave, aux.valor);
                
                this.insertar(cubeta.recuperar().clave, cubeta.recuperar().valor);
                //(talla++ es fa a this.insertar())
            }
    ...
    public final double desviacionTipica() {
        //Obtenir longitud mitjana de les cubetes.
        //La mitjana es el sumatori de ((mitjana - elementsCubeta)^ 2)/tamany
        double mitjana = factorCarga();
        double suma_quad = 0;
        
        //Calcular numerador (sumatori) de l'equacio
        for(int cub = 0; cub < elArray.length; cub++){
            //(Elements de cada cubeta - mitjana) al quadrat
            suma_quad += (elArray[cub].talla() - mitjana) * (elArray[cub].talla() - mitjana);
        }
        return Math.sqrt(suma_quad / elArray.length);  
    }
    ...
    public final double costeMLocalizar() {
        int colisiones = 0;
        for(int cub = 0; cub < elArray.length; cub++){
            //Colisions= NumElementsCubeta* (1+NumElementsCubeta) / 2
            colisiones += elArray[cub].talla()*(1+elArray[cub].talla())/ 2;
        }
