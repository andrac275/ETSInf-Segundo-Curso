...
public class BuscadorDeLaBibl {
    ...
    public ListaConPI<Termino> hapax() {
        /* COMPLETAR */
        //Llista per a emmagatzemar el resultat
        ListaConPI<Termino> res = new LEGListaConPI<Termino>();
        res.inicio();
        //Llista de claus de l'index
        ListaConPI<Termino> claus = index.claves();
                
        for(claus.inicio(); !claus.esFin(); claus.siguiente()){//Per cada clau a l'index
            //Recuperar llista de postings associada
            ListaConPI<BuscadorDeLaBibl.Posting> posts = index.recuperar(claus.recuperar());
            
            if (posts.talla() == 1){//si la llista es de talla ==1
                //inserir la clau al resultat 
                res.insertar(claus.recuperar());
            }
            
        }
        //si el resultat es de talla > 0
            //retornar resultat
        if (res.esVacia()){
            return null;
        }
