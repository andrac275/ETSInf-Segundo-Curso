...
devuelve_reloj_en_s_srd:
				...
				lbu $t0, 2($a0)		# $t0 == HH
				sll $v0, $t0,12		#$v0 = HH x 2^12
				sll $t1, $t0,9		#$t1 = HH x 2^9
				subu $v0, $v0, $t1	#$v0= HH x (2^12 - 2^9)
				sll $t1,$t0, 5		#$t1 = HH x 2^5
				addu $v0, $v0, $t1	#$v0= HH x (2^12 - 2^9 + 2^5)
				sll $t1,$t0, 4		#$t1 = HH x 2^4
				subu $v0, $v0, $t1	#$v0= HH x (2^12 - 2^9 + 2^5 - 2^4)

				#Multiplicar MM per 60
				lbu $t0, 1($a0)		# $t0 == MM
				sll $t1, $t0,6		#$v0 = MM x 2^6
				sll $t2, $t0,2		#$t1 = MM x 2^2
				subu $t1, $t1, $t2	#$v0= MM x (2^6 - 2^2)
